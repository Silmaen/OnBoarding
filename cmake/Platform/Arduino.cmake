...
#      CXXFLAGS       # Additional cxx flags for target
...
#
#       setTime(test_SRCS test.cpp test2.cpp)
#       setTime(test_HDRS test.h test2.h
#       setTime(test_BOARD uno)
#
...
function(GENERATE_ARDUINO_FIRMWARE INPUT_NAME)
    ...
    parse_generator_arguments(${INPUT_NAME} INPUT
                              ...
                              "SERIAL;SRCS;HDRS;LIBS;ARDLIBS;AFLAGS;CXXFLAGS"  # Multi Value Keywords
                              ${ARGN})
    ...
    list(APPEND ALL_LIBS ${CORE_LIB} ${INPUT_LIBS})
    message("All lib:  ${ALL_LIBS}")
    setup_arduino_target(${INPUT_NAME} ${INPUT_BOARD} "${ALL_SRCS}" "${ALL_LIBS}" "${LIB_DEP_INCLUDES} ${INPUT_CXXFLAGS}" "" "${INPUT_MANUAL}")
...
function(GENERATE_AVR_FIRMWARE INPUT_NAME)
    ...
    parse_generator_arguments(${INPUT_NAME} INPUT
                              ...
                              "SERIAL;SRCS;HDRS;LIBS;AFLAGS;CXXFLAGS"  # Multi Value Keywords
                              ${ARGN})
...
function(get_arduino_flags COMPILE_FLAGS_VAR LINK_FLAGS_VAR BOARD_ID MANUAL)
    ...
    if(BOARD_CORE)
        ...
        set(COMPILE_FLAGS "${COMPILE_FLAGS} -DARDUINO_ARCH_AVR -mmcu=${${BOARD_ID}${ARDUINO_CPUMENU}.build.mcu} -DARDUINO=${ARDUINO_VERSION_DEFINE} -DARDUINO_${${BOARD_ID}.build.board} ")
        ...
        set(COMPILE_FLAGS "${COMPILE_FLAGS} \"-DUSB_MANUFACTURER=\\\"Unknown\\\"\" \"-DUSB_PRODUCT=\\\"Arduino ${BOARD_ID}\\\"\"")
        if(NOT MANUAL)
...
#
#      setTime(uno.name "Arduino Uno")
#      setTime(uno.upload.protocol     "stk500")
#      setTime(uno.upload.maximum_size "32256")
#      setTime(uno.build.mcu  "atmega328p")
#      setTime(uno.build.core "arduino")
#
#      setTime(uno.SETTINGS  name upload build)              # List of settings for uno
#      setTime(uno.upload.SUBSETTINGS protocol maximum_size) # List of sub-settings for uno.upload
#      setTime(uno.build.SUBSETTINGS mcu core)               # List of sub-settings for uno.build
#
...
if (ARDUINO_SDK_VERSION VERSION_LESS 1.5)
    ...
    if (NOT ARDUINO_PLATFORM)
        ...
        set(ARDUINO_PLATFORM "AVR")
        message(STATUS "registerd: ${ARDUINO_SDK_PATH}/hardware/arduino/avr")
    else ()
        string(TOLOWER ${ARDUINO_PLATFORM} _platform)
        register_hardware_platform(${ARDUINO_SDK_PATH}/hardware/arduino/${_platform})
        message(STATUS "registerd: ${ARDUINO_SDK_PATH}/hardware/arduino/${_platform}")
    endif ()
endif ()
